...
try:
    ...
    raise ValueError(
        f"SEMSEARCH_EMBEDDING_DIM must be an integer (got {os.environ.get('SEMSEARCH_EMBEDDING_DIM')!r})"
    ) from e
...
if _TOKEN_EMBEDDING_URL:
    try:
        ...
        logger.warning(
            ...
            "the 'embedding' extra: pip install cq-server[semsearch] or uv sync --extra semsearch",
            exc_info=True,
        )
