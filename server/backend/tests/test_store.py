...
async def _insert_and_approve(store: RemoteStore, **overrides: Any) -> KnowledgeUnit:
...
class TestInsertAndGet:
    async def test_insert_and_retrieve(self, store: RemoteStore) -> None:
        unit = _make_unit()
        await store.insert(unit)
        retrieved = await store.get_any(unit.id)
        assert retrieved == unit

    async def test_insert_duplicate_raises(self, store: RemoteStore) -> None:
        unit = _make_unit()
        await store.insert(unit)
        with pytest.raises(sqlite3.IntegrityError):
            await store.insert(unit)

    async def test_returns_none_for_missing_id(self, store: RemoteStore) -> None:
        assert await store.get("ku_nonexistent") is None

    async def test_insert_with_empty_domains_raises(self, store: RemoteStore) -> None:
...
class TestUpdate:
    async def test_update_persists_changes(self, store: RemoteStore) -> None:
    ...
    async def test_update_missing_unit_raises(self, store: RemoteStore) -> None:
        unit = _make_unit()
        with pytest.raises(KeyError, match="Knowledge unit not found"):
            await store.update(unit)

    async def test_update_with_empty_domains_raises(self, store: RemoteStore) -> None:
...
class TestQuery:
    async def test_returns_matching_units(self, store: RemoteStore) -> None:
        unit = await _insert_and_approve(store, domains=["databases"])
        results = await store.query(["databases"])
        assert len(results) == 1
        assert results[0].id == unit.id

    async def test_returns_empty_for_no_match(self, store: RemoteStore) -> None:
        await _insert_and_approve(store, domains=["databases"])
        assert await store.query(["networking"]) == []

    async def test_language_filter_boosts_matching_units(self, store: RemoteStore) -> None:
    ...
    async def test_language_filter_includes_units_without_language(self, store: RemoteStore) -> None:
    ...
    async def test_framework_filter_includes_units_without_framework(self, store: RemoteStore) -> None:
    ...
    async def test_language_filter_ranks_matching_higher(self, store: RemoteStore) -> None:
    ...
    async def test_multiple_languages_boost_any_match(self, store: RemoteStore) -> None:
    ...
    async def test_multiple_frameworks_boost_any_match(self, store: RemoteStore) -> None:
...
class TestStats:
    ...
    async def test_count_after_inserts(self, store: RemoteStore) -> None:
        await store.insert(_make_unit(domains=["a"]))
        await store.insert(_make_unit(domains=["b"]))
        assert await store.count() == 2

    async def test_domain_counts(self, store: RemoteStore) -> None:
...
class TestTierColumn:
    async def test_tier_column_exists_after_migration(self, store: RemoteStore) -> None:
    ...
    async def test_tier_column_defaults_to_private_for_migration(self, store: RemoteStore) -> None:
    ...
    async def test_insert_populates_tier_from_unit(self, store: RemoteStore) -> None:
    ...
    async def test_update_syncs_tier_column(self, store: RemoteStore) -> None:
    ...
    async def test_counts_by_tier_empty(self, store: RemoteStore) -> None:
        """Empty store returns empty dict."""
        assert await store.counts_by_tier() == {}

    async def test_counts_by_tier_approved_only(self, store: RemoteStore) -> None:
    ...
    async def test_counts_by_tier_groups_correctly(self, store: RemoteStore) -> None:
...
class TestReviewStatus:
    async def test_inserted_unit_has_pending_status(self, store: RemoteStore) -> None:
        unit = _make_unit()
        await store.insert(unit)
...
class TestStatusFiltering:
    async def test_query_excludes_pending_units(self, store: RemoteStore) -> None:
        unit = _make_unit(domains=["api"])
        await store.insert(unit)
        results = await store.query(["api"])
        assert len(results) == 0

    async def test_query_returns_approved_units(self, store: RemoteStore) -> None:
        unit = _make_unit(domains=["api"])
        await store.insert(unit)
    ...
    async def test_query_excludes_rejected_units(self, store: RemoteStore) -> None:
        unit = _make_unit(domains=["api"])
        await store.insert(unit)
    ...
    async def test_get_only_returns_approved_for_agents(self, store: RemoteStore) -> None:
        unit = _make_unit()
        await store.insert(unit)
        assert await store.get(unit.id) is None

    async def test_get_returns_approved_unit(self, store: RemoteStore) -> None:
        unit = _make_unit()
        await store.insert(unit)
        await store.set_review_status(unit.id, "approved", "reviewer")
...
class TestReviewQueue:
    async def test_pending_queue_returns_pending_units(self, store: RemoteStore) -> None:
    ...
    async def test_daily_counts(self, store: RemoteStore) -> None:
        await store.insert(_make_unit(domains=["a"]))
        await store.insert(_make_unit(domains=["b"]))
    ...
    async def test_daily_counts_gap_fills_to_today(self, store: RemoteStore) -> None:
    ...
    async def test_daily_counts_includes_approved(self, store: RemoteStore) -> None:
    ...
    async def test_daily_counts_includes_rejected(self, store: RemoteStore) -> None:
    ...
    async def test_daily_counts_rejects_non_positive_days(self, store: RemoteStore) -> None:
    ...
    async def test_counts_by_status_empty(self, store: RemoteStore) -> None:
        counts = await store.counts_by_status()
        assert counts == {}
...
class TestApiKeys:
    ...
    async def test_create_and_fetch_active_by_id(self, store: RemoteStore) -> None:
    ...
    async def test_get_active_by_id_excludes_revoked(self, store: RemoteStore) -> None:
        user_id = await self._seed_user(store)
...
class TestEndToEnd:
    async def test_propose_confirm_flag_lifecycle(self, store: RemoteStore) -> None:
