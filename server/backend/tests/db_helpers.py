...
class _RepoBundle:
    ...
    async def update(self, unit: KnowledgeUnit) -> None:
        ...
        Parameters:
            unit (KnowledgeUnit): The knowledge unit with updated fields; must correspond to an existing
    ...
    async def daily_counts(self, *args, **kwargs):
        ...
        Parameters:
            ...
            **kwargs: Keyword arguments forwarded to the underlying reviews repository (e.g.,
                filters, date range, grouping options).

        Returns:
            A sequence of (date, count) pairs representing the number of reviews for each day
    ...
    async def pending_queue(self, *args, **kwargs):
        ...
        Accepts positional and keyword arguments used to filter or page results; these are passed
            through to the underlying review implementation.
...
def _make_store(db_path: Path) -> _RepoBundle:
    ...
    Parameters:
        db_path (Path): Filesystem path for the SQLite database file; used to construct the database
            URL and stored in settings.

    Returns:
        _RepoBundle: A test repository bundle whose repositories share a Database configured to use
            the provided SQLite file.
    """
