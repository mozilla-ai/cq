...
1. Call the `cq_status` MCP tool (no arguments needed).
...
- **Without `promoted_to_team`:** Display only: "The local cq store is empty. Knowledge units are added via `cq_propose` or the `/cq:reflect` command."
