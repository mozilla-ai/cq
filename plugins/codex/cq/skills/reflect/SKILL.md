...
The cq binary lives at the shared runtime path used by all host integrations:

- **Windows**: `$env:LOCALAPPDATA\cq\runtime\bin\cq.exe`
- **macOS / Linux**: `~/.local/share/cq/runtime/bin/cq`

If the platform-appropriate path does not exist, fall back to `which cq` / `where cq`.
...
Reflection is agent-led — there is no tool for this step. Using your own reasoning, scan the session for insights worth sharing.
...
- Insights already surfaced and confirmed during the session (i.e. knowledge units you retrieved via `cq query` and subsequently called `cq confirm` on to record that they proved correct).
- Insights you already proposed via `cq propose` during this session.
...
For each approved candidate, first run a quick `cq query` with the candidate's domains to check for close existing KUs. If a close match exists, consider `cq confirm` (same insight) or `cq flag` (contradicts it) instead of a new proposal. If no close match, call `cq propose`:
...
- **`cq propose` error** — Report the error inline for that candidate and continue with the next one. Do not abort.