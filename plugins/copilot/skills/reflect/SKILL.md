...
Reflection is agent-led — there is no CLI command for this step. Using your own reasoning, scan the session for insights worth sharing.
...
### Step 4.5 — Locating the cq binary
...
For each approved candidate, first run a quick `cq query` with the candidate's domains to check for close existing KUs. If a close match exists, consider `cq confirm` (same insight) or `cq flag` (contradicts it) instead of a new proposal. If no close match, run `cq propose`:
